...
const Leddetail = () => {
    ...
    useEffect(()=>{
     const rela = tvapi.filter((re)=>{
       return re.brand === leddet.brand
     })
     setRelated(rela)
    // eslint-disable-next-line react-hooks/exhaustive-deps
    },[tvapi])
    const dispatch = useDispatch()
    const leddet = tvapi.find((item)=>item.id == id)
    const detail = (leddet)=>{
        dispatch(addtocart(leddet))
    }
  return (
    <div>
   ...
   <div className="1img">
     <img src={leddet.image} className=' w-[400px] border-2 border-gray-200 p-2 h-[400px] max-sm:w-[200px] max-sm:h-[200px]' alt="" />
   </div>
   </div>
   <div className="laptop-content-detail w-1/3 max-xl:w-1/2 max-lg:w-3/4">
   <p className=' my-4 text-6xl'>{leddet.category}</p>
   ...
   <div className="price flex justify-start items-center my-4">
     <span className=' text-green-600'>Big Discount <span className=' text-red-600'>{leddet.discount}%</span> : </span>
     <p className=' line-through'>RS. {leddet.price}</p>
   </div>
   <div className="price flex justify-start items-center my-4">
     <span className=' text-green-600'>The Best Today Deal : </span>
     <p>RS. {Math.round(leddet.price*(1-leddet.discount/100))}</p>
   </div>
   <p className=' text-xl font-bold'>{leddet.title}</p>
   <p className=' my-4'>{leddet.description}</p>
   ...
   <p className=' my-4 text-xl'><span  className=' font-bold'>Brand :</span>{leddet.brand}</p>
   <p className=' my-4 text-xl'><span className=' font-bold'>Client_Says :</span> {leddet.happy}</p>
   <p className=' my-4 text-xl'><span className=' font-bold'>Warranty :</span>{leddet.warranty}</p>
   <p className=' my-4 text-xl'><span className=' font-bold'>QTY :</span>{leddet.qty}</p>
   <button className=' py-2 px-4 bg-blue-600 text-white hover:bg-blue-800 duration-200' onClick={()=>detail(leddet)}>Add To Cart</button>
   </div>
  </div>
