...
const Mobiledetail = () => {
    ...
    useEffect(()=>{
     const rela = mobileapi.filter((re)=>{
       return re.brand === mobdet.brand
     })
     setRelated(rela)
    // eslint-disable-next-line react-hooks/exhaustive-deps
    },[mobileapi])
    const dispatch = useDispatch()
  const mobdet = mobileapi.find((item)=>item.id == id)
  const [mainimage, setMainimage] = useState(mobdet.thumbnail[0])
  // const mobile = useSelector(selectcart)
  const addcartt = (mobdet)=>{
    dispatch(addtocart(mobdet))
  }
  return (
    <div>
      ...
      <p className=' my-4 text-6xl'>{mobdet.category}</p>
     ...
     <span className=' text-green-600'>Big Discount <span className=' text-red-600'>{mobdet.discountpercent}%</span> : </span>
     <p className=' line-through'>RS. {mobdet.price}</p>
   </div>
      <div className="price flex justify-start items-center my-4">
      
        <span className=' text-green-600'>The Best Today Deal : </span>
        <p>RS. {Math.round(mobdet.price*(1-mobdet.discountpercent/100))}</p>
      </div>
      <p className=' text-xl font-bold'>{mobdet.title}</p>
      <p className=' my-4'>{mobdet.description}</p>
      ...
      <p className=' my-4 text-xl'><span  className=' font-bold'>Brand :</span>{mobdet.brand}</p>
      <p className=' my-4 text-xl'><span className=' font-bold'>Client_Says :</span> {mobdet.happy}</p>
      <p className=' my-4 text-xl'><span className=' font-bold'>Warranty :</span>{mobdet.warranty}</p>
      <p className=' my-4 text-xl'><span className=' font-bold'>QTY :</span>{mobdet.qty}</p>
      <button className=' py-2 px-4 bg-blue-600 text-white hover:bg-blue-800 duration-200' onClick={()=>addcartt(mobdet)}>Add To Cart</button>
      </div>
     </div>
