...
const Laptop_detail = () => {
 ...
 useEffect(()=>{
  const rela = laptopapi.filter((re)=>{
    return re.brand === lapdet.brand
  })
  setRelated(rela)
 // eslint-disable-next-line react-hooks/exhaustive-deps
 },[laptopapi])
  const {id} = useParams()
  const lapdet = laptopapi.find((item)=>item.id == id)
  const [mainimage, setMainimage] = useState(lapdet.thumbnail[0])
  const addcart = (lapdet)=>{
    dispatch(addtocart(lapdet))
  }
  return (
    <div>
     <div className="laptop_detail_wrapper flex justify-around items-center gap-6 py-28 max-lg:flex-col">
      ...
      <div className="2img">
        {lapdet.thumbnail.map((x)=>{
          return <img src={x} onClick={()=>setMainimage(x)} className=' w-[100px] border-2 border-black p-2 h-[100px]' alt='' />
        })}
      </div>
      <div className="1img">
        <img src={mainimage} className=' w-[400px] border-2 border-black p-2 h-[400px] max-sm:w-[200px] max-sm:h-[200px]' alt="" />
      </div>
      </div>
      <div className="laptop-content-detail w-1/3 max-xl:w-1/2 max-lg:w-3/4">
      <p className=' my-4 text-6xl'>{lapdet.category}</p>
     ...
     <span className=' text-green-600'>Big Discount <span className=' text-red-600'>{lapdet.discountpercent}%</span> : </span>
     <p className=' line-through'>RS. {lapdet.price}</p>
   </div>
      <div className="price flex justify-start items-center my-4">
      
        <span className=' text-green-600'>The Best Today Deal : </span>
        <p>RS. {Math.round(lapdet.price*(1-lapdet.discountpercent/100))}</p>
      </div>
      <p className=' text-xl font-bold'>{lapdet.title}</p>
      <p className=' my-4'>{lapdet.description}</p>
      ...
      <p className=' my-4 text-xl'><span  className=' font-bold'>Brand :</span>{lapdet.brand}</p>
      <p className=' my-4 text-xl'><span className=' font-bold'>Client_Says :</span> {lapdet.happy}</p>
      <p className=' my-4 text-xl'><span className=' font-bold'>Warranty :</span>{lapdet.warranty}</p>
      <p className=' my-4 text-xl'><span className=' font-bold'>QTY :</span>{lapdet.qty}</p>
      <button className=' py-2 px-4 bg-blue-600 text-white hover:bg-blue-800 duration-200' onClick={()=>addcart(lapdet)}>Add To Cart</button>
      </div>
     </div>
